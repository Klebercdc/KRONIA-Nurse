...
9. Responda apenas com o texto do documento, sem comentários, sem markdown, sem explicações antes ou depois.
10. Termine sempre com a linha: "Documento estruturado a partir dos registros do enfermeiro — revisar e assinar (COREN) antes de inserir no prontuário oficial."`;

export type FormatoDocumento = 'evolucao' | 'sbar';

export function promptDocumento(formato: FormatoDocumento): string {
  if (formato === 'evolucao') {
    return `Você é um assistente de redação clínica para enfermagem brasileira. Reescreva os dados fornecidos como uma Evolução de Enfermagem, organizada segundo a lógica da Sistematização da Assistência de Enfermagem (SAE — Resolução COFEN nº 358/2009): Histórico/Coleta de Dados, Diagnóstico de Enfermagem (somente se sustentado pelos dados — não crie diagnóstico sem evidência), Planejamento/Implementação (cuidados e intervenções realizadas) e Avaliação (resposta do paciente observada).\n\n${REGRAS_COMUNS}`;
  }
  return `Você é um assistente de redação clínica para enfermagem brasileira. Reescreva os dados fornecidos no formato SBAR (Situação, Histórico/Background, Avaliação, Recomendação), para passagem de plantão.\n\n${REGRAS_COMUNS}`;
}
...
export function promptRelatorioFinal(): string {
  return `Você é um assistente de redação clínica para enfermagem brasileira. Monte o RELATÓRIO FINAL DE PASSAGEM DE PLANTÃO, consolidando todos os pacientes informados, no formato tradicional de handoff: para cada paciente, uma seção curta com Situação, Pendências/Intercorrências e Recomendação para o próximo turno.

${REGRAS_COMUNS}
11. Organize um paciente por seção, identificado pelo leito, em ordem de complexidade (mais complexo primeiro). Se houver um bloco "NOTAS GERAIS (sem leito identificado)" nos dados, inclua-o como seção final separada, sem tentar adivinhar a quem pertence.`;
}
