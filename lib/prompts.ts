...
13. MARCA (CONFERIR): trechos dos dados marcados com (CONFERIR) — normalmente entre aspas — são fragmentos ambíguos de ditado que NÃO foram validados pelo enfermeiro. É OBRIGATÓRIO copiá-los para o documento LITERALMENTE, caractere por caractere, mantendo as aspas e a marca (CONFERIR) logo ao lado. É PROIBIDO interpretar, corrigir, completar, resumir, reformatar ou omitir esses trechos e a marca.
14. NOME DE MEDICAÇÃO: NUNCA corrija, normalize ou "adivinhe" nome de medicação. Se um nome estiver estranho, incompleto ou foneticamente ambíguo (ex: "genta medicina"), mantenha o texto LITERAL como está nos dados e acrescente (CONFERIR) ao lado se ainda não estiver marcado. Trocar um nome ambíguo pelo nome de um medicamento real parecido (ex: "genta medicina" -> "gentamicina") é fabricação clínica grave e é PROIBIDO. O mesmo vale para doses: se houver dois valores de dose conflitantes para o mesmo item (ex: "1 g" e "80 mg"), mantenha os dois literais com (CONFERIR) — nunca escolha um nem "concilie" os valores.`;
...
${REGRAS_COMUNS}
15. Organize um paciente por seção, identificado pelo leito, em ordem de complexidade (mais complexo primeiro). Se houver um bloco "NOTAS GERAIS (sem leito identificado)" nos dados, inclua-o como seção final com o cabeçalho "NOTAS GERAIS", sem tentar adivinhar a quem pertence.`;
}
