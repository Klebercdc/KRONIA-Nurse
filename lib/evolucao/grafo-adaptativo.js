...
/**
 * Rótulo devolvido por classify() quando o valor medido está na faixa de
 * referência. Fica só em `label` — que é apoio de tela — porque o COREN-SP
 * proíbe registrar sinal vital como "normocárdico"/"normotenso".
 *
 * É constante exportada, e não string solta em cada classify, para que quem
 * monta o documento consiga perguntar "este achado está alterado?" sem
 * comparar texto por conta própria.
 */
const LABEL_NA_FAIXA = "dentro da faixa de referência";

/** True quando classify() apontou alteração, e não valor dentro da faixa. */
function achadoAlterado(resultado) {
  return Boolean(resultado && resultado.label && !resultado.label.startsWith(LABEL_NA_FAIXA));
}

// Reexecuta o classify() da própria pergunta (via CONTEXTS, resolvido em
// runtime — a função só roda depois que CONTEXTS já está montado) para
// checagens cruzadas de sinal vital sem duplicar threshold clínico em
// segundo lugar. Fonte de cada limiar continua sendo só o classify.
function classifyQuestion(id, ...args) {
  const ctx = CONTEXTS.find((c) => c.id === "evolucao_universal");
  const q = ctx && ctx.questions.find((qq) => qq.id === id);
  return q && q.classify ? q.classify(...args) : null;
}

function fcAlterado(answers) {
  if (answers.fc === undefined || answers.fc === "") return false;
  const v = Number(answers.fc);
  return !Number.isNaN(v) && achadoAlterado(classifyQuestion("fc", v, answers));
}
function frAlterado(answers) {
  if (answers.fr === undefined || answers.fr === "") return false;
  const v = Number(answers.fr);
  return !Number.isNaN(v) && achadoAlterado(classifyQuestion("fr", v, answers));
}
function spo2Alterado(answers) {
  if (answers.spo2 === undefined || answers.spo2 === "") return false;
  const v = Number(answers.spo2);
  return !Number.isNaN(v) && achadoAlterado(classifyQuestion("spo2", v));
}
function temperaturaAlterado(answers) {
  if (answers.temperatura === undefined || answers.temperatura === "") return false;
  const v = Number(answers.temperatura);
  return !Number.isNaN(v) && achadoAlterado(classifyQuestion("temperatura", v, answers));
}
function glicemiaAlterado(answers) {
  if (answers.glicemia === undefined || answers.glicemia === "") return false;
  const v = Number(answers.glicemia);
  return !Number.isNaN(v) && achadoAlterado(classifyQuestion("glicemia", v));
}
function paAlterado(answers) {
  const pa = answers.pa;
  if (!pa || pa.a === undefined || pa.a === "" || pa.b === undefined || pa.b === "") return false;
  const s = Number(pa.a), d = Number(pa.b);
  return !Number.isNaN(s) && !Number.isNaN(d) && achadoAlterado(classifyQuestion("pa", s, d, answers));
}
/** True se qualquer sinal vital universal estiver fora da faixa de referência. */
function algumVitalAlterado(answers) {
  return (
    fcAlterado(answers) || frAlterado(answers) || spo2Alterado(answers) ||
    temperaturaAlterado(answers) || glicemiaAlterado(answers) || paAlterado(answers)
  );
}
...
const CONTEXTS = [
  {
    ...
    validacoesCruzadas: [
      ...
      // ---- P0: genérica — estado geral x sinal vital ----
      {
        check: (answers) => answers.estado_geral === "normal" && algumVitalAlterado(answers),
        mensagem: "estado geral registrado como preservado, sem sinais de sofrimento, mas há sinal vital fora da faixa de referência (FC/FR/PA/SpO₂/temperatura/glicemia) — revisar coerência entre estado geral e sinais vitais",
      },
      // ---- P0: caso relatado — conduta de O₂ sem via aérea que a suporte ----
      {
        check: (answers) =>
          spo2Alterado(answers) &&
          answers.conduta_hipoxemia === "aumento_fluxo" &&
          (answers.via_aerea === "ar_ambiente" || answers.via_aerea_neo === "ar_ambiente"),
        mensagem: "conduta 'aumento de fluxo/FiO₂' registrada com paciente em ar ambiente — não é possível aumentar fluxo de suporte de oxigênio inexistente, revisar via aérea ou conduta",
      },
      // ---- P0: mutualidade de "nenhum"/"nenhuma" em multi_select ----
      {
        check: (answers) => Array.isArray(answers.dispositivos) && answers.dispositivos.includes("nenhum") && answers.dispositivos.length > 1,
        mensagem: "dispositivos: 'Nenhum' selecionado junto com outro(s) dispositivo(s) — combinação inconsistente, revisar",
      },
      {
        check: (answers) => Array.isArray(answers.dispositivos_neo) && answers.dispositivos_neo.includes("nenhum") && answers.dispositivos_neo.length > 1,
        mensagem: "dispositivos (neonatal): 'Nenhum' selecionado junto com outro(s) acesso(s) — combinação inconsistente, revisar",
      },
      {
        check: (answers) =>
          Array.isArray(answers.dor_toracica_caracteristicas) &&
          answers.dor_toracica_caracteristicas.includes("nenhuma") &&
          answers.dor_toracica_caracteristicas.length > 1,
        mensagem: "características da dor torácica: 'Nenhuma característica associada' selecionada junto com outra(s) — combinação inconsistente, revisar",
      },
      // ---- P1: balanço hídrico/hidratação x edema de membros ----
      {
        check: (answers) => {
          const bh = answers.balanco_hidrico;
          const positivoGrande =
            bh && bh.a !== undefined && bh.a !== "" && bh.b !== undefined && bh.b !== "" &&
            Number(bh.a) - Number(bh.b) > 1500;
          const semEdemaMembros = answers.mmss === "normal" && answers.mmii === "normal";
          return (positivoGrande || answers.hidratacao === "edemaciado") && semEdemaMembros;
        },
        mensagem: "balanço hídrico fortemente positivo ou estado de hidratação 'edemaciado' registrado, mas membros superiores e inferiores sem edema — checar consistência entre os três campos",
      },
      // ---- P1: escore RASS x nível de consciência ----
      {
        check: (answers) => {
          const rass = answers.sedacao_rass;
          if (rass === undefined) return false;
          if (["+4", "+3", "+2", "+1", "0"].includes(rass) && ["sedado", "comatoso"].includes(answers.consciencia)) return true;
          if (["-4", "-5"].includes(rass) && ["sonolento", "confuso"].includes(answers.consciencia)) return true;
          return false;
        },
        mensagem: "escore RASS registrado incompatível com o nível de consciência informado — revisar",
      },
      // ---- P2: parâmetro ventilatório respondido sem via aérea = VM (defensivo, protege contra resposta anterior alterada) ----
      {
        check: (answers) =>
          (answers.fio2 !== undefined || answers.peep !== undefined || answers.modo_ventilatorio !== undefined) &&
          answers.via_aerea !== "vm",
        mensagem: "parâmetro ventilatório (FiO₂/PEEP/modo) respondido, mas via aérea não está registrada como ventilação mecânica invasiva — revisar",
      },
      {
        check: (answers) =>
          (answers.fio2_neo !== undefined || answers.peep_neo !== undefined || answers.modo_ventilatorio_neo !== undefined) &&
          answers.via_aerea_neo !== "vm",
        mensagem: "parâmetro ventilatório neonatal (FiO₂/PEEP/modo) respondido, mas via aérea não está registrada como ventilação mecânica invasiva — revisar",
      },
      // ---- P2: detalhe de dispositivo respondido sem o dispositivo marcado (defensivo) ----
      {
        check: (answers) =>
          (answers.avp_calibre !== undefined || answers.avp_local !== undefined || answers.avp_sitio_condicao !== undefined) &&
          !(Array.isArray(answers.dispositivos) && answers.dispositivos.includes("avp")),
        mensagem: "detalhe de acesso venoso periférico respondido sem AVP marcado em dispositivos — revisar",
      },
      {
        check: (answers) =>
          (answers.cvc_sitio !== undefined || answers.cvc_sitio_condicao !== undefined) &&
          !(Array.isArray(answers.dispositivos) && answers.dispositivos.includes("cvc")),
        mensagem: "detalhe de cateter venoso central respondido sem CVC marcado em dispositivos — revisar",
      },
      {
        check: (answers) =>
          (answers.dreno_debito !== undefined || answers.dreno_aspecto !== undefined || answers.dreno_sitio_condicao !== undefined) &&
          !(Array.isArray(answers.dispositivos) && answers.dispositivos.includes("dreno")),
        mensagem: "detalhe de dreno respondido sem dreno marcado em dispositivos — revisar",
      },
      // ---- P2: sedativo respondido sem consciência compatível (defensivo) ----
      {
        check: (answers) =>
          answers.sedativo_qual !== undefined && !["sedado", "comatoso"].includes(answers.consciencia),
        mensagem: "medicação sedativa registrada sem nível de consciência compatível (sedado/comatoso) — revisar",
      },
    ],
    questions: [
      ...
      {
        id: "ferida_operatoria_local", layer: "condicional",
        showIf: { questionId: "situacao_clinica", equals: "cirurgico" }, type: "texto_livre",
        titulo: "Qual a localização da ferida operatória?",
        placeholder: "Ex: xifopúbica, subcostal direita, inguinal esquerda",
        montarFrase: (texto) => `em região ${texto}`,
      },
      {
        // Só na deiscência. Ferida fechada e seca não se mensura de rotina:
        // a característica que muda a conduta ali é o aspecto, já perguntado
        // acima. Deiscência aberta, sim — é o tamanho que diz se abriu mais
        // desde a evolução anterior.
        id: "ferida_operatoria_tamanho", layer: "condicional",
        showIf: { questionId: "ferida_operatoria", equals: "deiscencia" }, type: "numeric_pair",
        titulo: "Qual o tamanho da deiscência?",
        unitA: "cm", labelA: "Comprimento", minA: 0.1, maxA: 60,
        unitB: "cm", labelB: "Largura", minB: 0.1, maxB: 60,
        // Sem repetir "deiscência": a opção acima já nomeou o achado, e esta
        // frase vem colada nela.
        classify: (c, l) => ({ label: null, frase: `medindo ${c} x ${l} cm` }),
      },
      ...
      },
      {
        ...
        classify: (v) =>
          ...
          : { label: LABEL_NA_FAIXA, frase: `com BCF de ${v} bpm` },
      },
      {
      ...
      },
      {
        ...
        options: [
          { value: "adequada", label: "Compatível com o tempo de puerpério", frase: "involução uterina compatível com o tempo de puerpério" },
          { value: "inadequada", label: "Útero subinvoluído", frase: "útero subinvoluído para o tempo de puerpério" },
      ...
      },
      {
        ...
        options: [
          { value: "pega_adequada", label: "Sem dificuldade de pega", frase: "amamentando sem dificuldade de pega" },
      ...
      },
      {
        ...
        options: [
          ...
          { value: "episiotomia_cicatrizando", label: "Episiotomia/laceração em cicatrização", frase: "episiotomia em processo de cicatrização, sem sinais inflamatórios" },
      ...
      // ---- Condicionais de sedação (adulto — dispara por "sedado"/"comatoso") ----
      {
        ...
        // Nenhuma opção escreve frase: marcar a droga aqui SEMPRE abre a
        // pergunta de dose dela, e é a frase da dose que a nomeia junto do
        // valor ("midazolam em infusão contínua a 12 mL/h"). Se as duas
        // escrevessem, o texto saía "midazolam, midazolam em infusão
        // contínua a 12 mL/h".
        //
        // O nome fica na frase da dose, e não o contrário, porque com duas
        // drogas em curso é ele que amarra cada dose à sua droga.
        // Dose não respondida não perde a informação: a pendência é
        // "(CONFERIR — Qual a dose de midazolam? não respondido)".
        options: [
          { value: "midazolam", label: "Midazolam (Dormonid)", frase: null },
          { value: "propofol", label: "Propofol", frase: null },
          { value: "fentanil", label: "Fentanil", frase: null },
          { value: "dexmedetomidina", label: "Dexmedetomidina", frase: null },
      ...
      },
      {
        ...
        // Os descritores abaixo são a redação oficial da Richmond
        // Agitation-Sedation Scale, com o escore sempre junto. "Muito
        // agitado" e "sedação leve" ficam como estão: aqui não são juízo de
        // valor do profissional, são o nome do degrau de um instrumento
        // validado — que é justamente o que o COREN-SP manda registrar,
        // "incluindo valor do escore aferido".
        options: [
      ...
      // ---- Sinais vitais — UMA pergunta cada, classificação se adapta à idade ----
      {
        ...
        // COREN-SP, "Anotação de Enfermagem" (2022): "os sinais vitais
        // mensurados devem ser registrados pontualmente, ou seja, os valores
        // exatos aferidos. Não registrar como 'normotenso', 'normocárdico',
        // etc". Por isso o valor dentro da faixa vai para o prontuário SEM
        // rótulo — só o número. O rótulo de normalidade sobrevive apenas em
        // `label`, que é feedback de tela e não entra no documento.
        //
        // O achado ALTERADO continua nomeado: é sinal clínico observado, que
        // o mesmo documento manda registrar, e não julgamento de normalidade.
        classify: (v, answers) => {
          ...
          return { label: `${LABEL_NA_FAIXA}${rotulo}`, frase: `com frequência cardíaca de ${v} bpm` };
        },
      },
      // ---- Condutas — sinais de alerta (idade-conscientes onde precisa) ----
      //
      // As frases de conduta repetem o achado de propósito ("...sobre a
      // alteração de frequência cardíaca", "...frente ao quadro febril"), e
      // isso NÃO é redundância a ser limpa: conduta sai do parágrafo do
      // achado e vai para o bloco Intervenção, onde precisa se sustentar
      // sozinha. Tirar o substantivo deixaria "Equipe médica comunicada." sem
      // dizer sobre o quê.
      //
      // Mesma razão para o nome do dispositivo aparecer nas frases de
      // condição do sítio: com AVP, cateter central e dreno juntos, é ele que
      // amarra cada achado ao seu dispositivo.
      {
      ...
      },
      {
        ...
        classify: (sist, diast, answers) => {
          ...
          // "normal" e "ótima" são exatamente o rótulo que o COREN-SP proíbe
          // ("Não registrar como 'normotenso'"): nessas duas faixas o
          // prontuário recebe só o valor aferido. As faixas de hipertensão
          // seguem nomeadas — são o achado alterado, não juízo de normalidade.
          const categorias = [
          ...
          if (!achado) {
            return { label: LABEL_NA_FAIXA, frase: `com pressão arterial de ${sist}x${diast} mmHg${notaMeta}` };
          }
          return { label: achado.nome, frase: `${achado.nome}, com pressão arterial de ${sist}x${diast} mmHg${notaMeta}` };
        },
      },
      {
      ...
      },
      {
        ...
        classify: (v, answers) => {
          ...
          return { label: `${LABEL_NA_FAIXA}${rotulo}`, frase: `com frequência respiratória de ${v} irpm` };
        },
      },
      {
        ...
        classify: (v) =>
          // Adjetivo, para o rótulo servir tanto de apoio na tela quanto de
          // descritor na abertura da evolução ("Apresenta-se hipoxêmico").
          v < 90 ? { label: "hipoxêmico", frase: `apresentando hipoxemia, com SpO₂ de ${v}%` }
          // "discretamente" é termo de conotação de valor, vedado pelo
          // COREN-SP. O que fica é o valor aferido e o alvo objetivo.
          : v < 95 ? { label: "com SpO₂ abaixo do alvo de 95%", frase: `com SpO₂ de ${v}%, abaixo do alvo de 95%` }
          : { label: LABEL_NA_FAIXA, frase: `com SpO₂ de ${v}%` },
      },
      {
      ...
      },
      {
        ...
        classify: (v, answers) => {
          ...
          if (neonatal) {
            ...
            return { label: LABEL_NA_FAIXA, frase: `com temperatura axilar de ${v}°C` };
          }
      ...
      // ---- Dor (adulto — EVA) / Desconforto respiratório (neonatal — Silverman) ----
      {
        ...
        options: [
          // COREN-SP manda registrar "dados de aplicação de Escala de dor
          // (...) incluindo valor do escore aferido". Como a pergunta é por
          // faixa e não por número exato, o registro leva a faixa da EVA.
          { value: "sem_dor", label: "Sem dor (0)", frase: "nega dor, escore EVA 0" },
          { value: "leve", label: "Leve (1–3)", frase: "refere dor, escore EVA entre 1 e 3" },
          { value: "moderada", label: "Moderada (4–6)", frase: "refere dor, escore EVA entre 4 e 6" },
          { value: "intensa", label: "Intensa (7–10)", frase: "refere dor, escore EVA entre 7 e 10" },
        ],
      },
      // ---- Dor torácica (adulto) ----
      {
      ...
      },
      {
        ...
        options: [
          ...
          { value: "leve", label: "Leve — tiragem intercostal", frase: "com tiragem intercostal" },
          ...
          { value: "grave", label: "Grave — gemido expiratório audível", frase: "com gemido expiratório audível, tiragem intercostal e batimento de asa de nariz" },
        ],
      },
      {
      ...
      {
        // O caminho neonatal sempre perguntou coloração da pele; o adulto só
        // perguntava integridade. O COREN-SP lista "coloração da pele" entre
        // as condições gerais a anotar, sem restringir por idade — palidez,
        // icterícia e cianose são achados de adulto tanto quanto de RN.
        id: "pele_coloracao", layer: "condicional",
        showIf: { questionId: "idade_unidade", equals: "anos" }, type: "select",
        titulo: "Como está a coloração da pele e mucosas?",
        options: [
          // Adjetivos, e não "pele e mucosas coradas": esta frase abre a
          // evolução depois de "Apresenta-se", onde só adjetivo concorda.
          { value: "corada", label: "Corada", frase: "corado" },
          { value: "palida", label: "Pálida/descorada", frase: "descorado" },
          { value: "ictericia", label: "Ictérica", frase: "ictérico" },
          { value: "cianose", label: "Cianose (central ou periférica)", frase: "cianótico" },
          { value: "marmorea", label: "Moteada/marmórea", frase: "com pele de aspecto marmóreo" },
        ],
      },
      {
        // Estado de hidratação entra na abertura da evolução, junto com
        // coloração — é o par clássico "corado e hidratado". Vale em todas as
        // idades, por isso não filtra por unidade.
        id: "hidratacao", layer: "condicional",
        showIf: { questionId: "idade_unidade", equals: ["anos", "meses", "dias"] }, type: "select",
        titulo: "Como está o estado de hidratação?",
        options: [
          { value: "hidratado", label: "Hidratado", frase: "hidratado" },
          { value: "desidratado", label: "Mucosas secas / turgor diminuído", frase: "com mucosas secas e turgor cutâneo diminuído" },
          { value: "edemaciado", label: "Edemaciado / hiper-hidratado", frase: "edemaciado" },
        ],
      },
      {
      ...
      },
      {
        ...
        // Sem repetir "lesão por pressão": a opção de `pele` já nomeou o
        // achado, e esta frase vem colada nela.
        options: [
          { value: "1", label: "Estágio I", frase: "estágio I" },
          { value: "2", label: "Estágio II", frase: "estágio II" },
          { value: "3", label: "Estágio III", frase: "estágio III" },
          { value: "4", label: "Estágio IV", frase: "estágio IV" },
          { value: "nao_classificavel", label: "Não classificável", frase: "não classificável" },
        ],
      },
      // Localização e tamanho da lesão.
      //
      // COREN-SP: os registros "devem priorizar a descrição de
      // características, como tamanho mensurado (cm, mm, etc.), quantidade
      // (ml, l, etc.), coloração e forma". Estágio sozinho não é
      // característica mensurada — e é o tamanho que permite comparar a
      // lesão com a evolução anterior e dizer se progrediu.
      {
        id: "pele_lesao_local", layer: "condicional",
        showIf: { questionId: "pele", equals: "lesao" }, type: "texto_livre",
        titulo: "Em que região está a lesão?",
        placeholder: "Ex: região sacral, calcâneo direito, trocanter esquerdo",
        montarFrase: (texto) => `em ${texto}`,
      },
      {
        id: "pele_lesao_tamanho", layer: "condicional",
        showIf: { questionId: "pele", equals: "lesao" }, type: "numeric_pair",
        titulo: "Qual o tamanho da lesão?",
        unitA: "cm", labelA: "Comprimento", minA: 0.1, maxA: 60,
        unitB: "cm", labelB: "Largura", minB: 0.1, maxB: 60,
        classify: (c, l) => ({ label: null, frase: `medindo ${c} x ${l} cm` }),
      },
      ...
      {
        // O dreno já registrava aspecto do débito; a diurese só registrava
        // volume. O COREN-SP pede as características — coloração inclusive —
        // e os exemplos do próprio documento registram assim ("débito
        // amarelo-claro").
        id: "diurese_aspecto", layer: "condicional",
        showIf: { questionId: "eliminacoes", equals: "sonda_vesical" }, type: "select",
        titulo: "Qual o aspecto da diurese?",
        options: [
          { value: "amarelo_claro", label: "Amarelo-clara, límpida", frase: "de aspecto amarelo-claro e límpido" },
          { value: "concentrada", label: "Concentrada", frase: "de aspecto concentrado" },
          { value: "hematurica", label: "Hematúrica", frase: "de aspecto hematúrico" },
          { value: "turva", label: "Turva/com sedimentos", frase: "de aspecto turvo, com sedimentos" },
        ],
      },
      {
      ...
      },
      {
        ...
        options: [
          // "boa tolerância" é conotação de valor. O que se registra é o
          // observável: a ausência dos achados que as outras opções nomeiam.
          { value: "bem_tolerada", label: "Sem resíduo, vômitos ou distensão", frase: "sem resíduo gástrico aumentado, vômitos ou distensão abdominal" },
      ...
      },
      {
        ...
        options: [
          { value: "adequada", label: "TEC < 3s, extremidades aquecidas", frase: "com tempo de enchimento capilar inferior a 3 segundos e extremidades aquecidas" },
      ...
      // ---- Condição do sítio de inserção ----
      //
      // COREN-SP, "Anotação de Enfermagem" (2022), duas exigências distintas
      // que caíam fora do schema:
      //
      //   - nas condições gerais: "dispositivos em uso (ex.: cateteres e como
      //     se encontram suas inserções e fixações; curativos e seu aspecto
      //     visível externamente)";
      //   - no item específico de acesso venoso periférico: "condições do
      //     local da punção (pele e rede venosa local)".
      //
      // Registrar que existe um cateter sem dizer como está o sítio deixa de
      // fora justamente o que muda a conduta. As opções descrevem achados
      // observáveis — sem "bom", "adequado" ou equivalente.
      //
      // Não abrimos pergunta de conduta atrelada a estas: a providência
      // adotada frente a um achado de sítio entra no fecho do documento, e
      // acrescentar mais um ramo por dispositivo alongaria o questionário sem
      // acrescentar dado que o COREN exija.
      {
        id: "avp_sitio_condicao", layer: "condicional",
        showIf: { questionId: "dispositivos", includes: "avp" }, type: "select",
        titulo: "Como está o local da punção do acesso venoso periférico?",
        options: [
          { value: "sem_alteracao", label: "Sem hiperemia, edema, dor ou secreção; curativo íntegro", frase: "com sítio de punção sem hiperemia, edema, dor à palpação ou secreção, curativo íntegro e fixado" },
          { value: "hiperemia", label: "Hiperemia no sítio de punção", frase: "com hiperemia no sítio de punção" },
          { value: "infiltracao", label: "Edema/infiltração", frase: "com edema e infiltração no sítio de punção" },
          { value: "secrecao", label: "Secreção no sítio de punção", frase: "com secreção no sítio de punção" },
          { value: "curativo_comprometido", label: "Curativo solto, sujo ou sem data", frase: "com curativo do acesso solto, sujo ou sem datação" },
        ],
      },
      {
      ...
      {
        id: "cvc_sitio_condicao", layer: "condicional",
        showIf: { questionId: "dispositivos", includes: "cvc" }, type: "select",
        titulo: "Como está o sítio de inserção do cateter venoso central?",
        options: [
          { value: "sem_alteracao", label: "Sem hiperemia, edema ou secreção; curativo íntegro", frase: "com sítio de inserção sem hiperemia, edema ou secreção, curativo íntegro e fixado" },
          { value: "hiperemia", label: "Hiperemia no sítio de inserção", frase: "com hiperemia no sítio de inserção do cateter central" },
          { value: "secrecao", label: "Secreção no sítio de inserção", frase: "com secreção no sítio de inserção do cateter central" },
          { value: "curativo_comprometido", label: "Curativo solto, sujo ou sem data", frase: "com curativo do cateter central solto, sujo ou sem datação" },
        ],
      },
      {
      ...
      {
        id: "dreno_sitio_condicao", layer: "condicional",
        showIf: { questionId: "dispositivos", includes: "dreno" }, type: "select",
        titulo: "Como está o sítio de inserção do dreno?",
        options: [
          { value: "sem_alteracao", label: "Sem hiperemia, edema ou extravasamento; curativo íntegro", frase: "com sítio de inserção do dreno sem hiperemia, edema ou extravasamento, curativo íntegro e fixado" },
          { value: "hiperemia", label: "Hiperemia no sítio de inserção", frase: "com hiperemia no sítio de inserção do dreno" },
          { value: "extravasamento", label: "Extravasamento pericateter", frase: "com extravasamento pericateter no sítio de inserção do dreno" },
          { value: "curativo_comprometido", label: "Curativo solto, sujo ou saturado", frase: "com curativo do dreno solto, sujo ou saturado" },
        ],
      },
      {
      ...
      },
      {
        ...
        classify: (v) =>
          ...
          : { label: LABEL_NA_FAIXA, frase: `glicemia capilar de ${v} mg/dL` },
      },

      // ---- Fecho do Processo de Enfermagem (Resolução COFEN nº 736/2024) ----
      //
      // DIAGNÓSTICO E PLANEJAMENTO NÃO SÃO COLETADOS AQUI, e isso é decisão de
      // produto, não esquecimento. Os dois são registrados no sistema próprio
      // de cada hospital, em documento à parte — o planejamento (prescrição de
      // enfermagem, o que a SAE antiga chamava assim) pela mesma razão que o
      // diagnóstico. A Res. 736/2024 exige o registro de todas as etapas no
      // prontuário (Art. 8º) — o prontuário, não necessariamente este
      // documento.
      //
      // Se um dia mudar: reintroduzir é acrescentar aqui um par de perguntas
      // como o de Resultado, abaixo, e a fatia correspondente em
      // processo-enfermagem.js. Enquanto não mudar, a ausência é verificada
      // por teste — não é para ser "consertada" numa próxima auditoria.
      //
      // Resultado é TEXTO LIVRE de propósito: o motor não deduz a resposta do
      // paciente a partir de achado.
      //
      // O "Sim/Não" antes do campo separa duas coisas diferentes: "não há o que
      // registrar" (resposta explícita do profissional) de "ninguém avaliou
      // isso" (pergunta não respondida, que vira pendência).
      {
        id: "tem_resultado", layer: "universal", type: "select",
        titulo: "Há resposta do paciente às intervenções para registrar?",
        options: [
          { value: "sim", label: "Sim", frase: null },
          { value: "nao", label: "Não", frase: null },
        ],
      },
      {
        id: "resultado_avaliacao", layer: "condicional",
        showIf: { questionId: "tem_resultado", equals: "sim" }, type: "texto_livre",
        titulo: "Qual a resposta do paciente após as intervenções?",
        placeholder: "Ex: afebril após antitérmico; refere alívio da dor",
      },
    ],
  },
];
...
/**
 * Como o paciente é chamado na abertura da evolução, pela faixa etária.
 *
 * Período neonatal é até 28 dias completos (OMS / Ministério da Saúde);
 * de 29 dias a 2 anos, lactente. Chamar um bebê de 15 dias de "paciente" não
 * está errado em português, mas perde a informação que o próprio termo
 * carrega na documentação de enfermagem.
 *
 * CONFERIR se o COREN de referência tiver norma própria de redação — a
 * classificação etária acima é consolidada, a exigência do termo na evolução
 * não foi verificada em resolução.
 */
function comoChamar(answers) {
  if (!answers) return "Paciente";
  if (answers.idade_unidade === "dias") {
    const dias = Number(answers.idade_dias);
    // O schema aceita até 29 dias, então o 29º já saiu do período neonatal.
    return !Number.isNaN(dias) && dias > 28 ? "Lactente" : "Recém-nascido";
  }
  if (answers.idade_unidade === "meses") return "Lactente";
  return "Paciente";
}

// Abertura de cada parágrafo. Onde é texto fixo, vale sempre; onde é função,
// recebe os subgrupos que o parágrafo realmente tem e as respostas, e decide.
//
...
const ABERTURA_PARAGRAFO = {
  geral: (subgrupos, answers) => comoChamar(answers),
  exame_fisico: "Ao exame físico,",
  abdome: null,
  // Quem está em ar ambiente NÃO está em suporte respiratório: anunciar o
  // parágrafo como "Em suporte respiratório, em ar ambiente" se contradiz na
  // mesma frase. A abertura passa a depender da via aérea respondida — e
  // sedação sem suporte ventilatório abre por si mesma.
  //
  // Via aérea não respondida também não abre: o motor não anuncia suporte
  // que não sabe se existe. A pendência (CONFERIR) é que cobra a resposta.
  suporte: (subgrupos, answers) => {
    const via = answers ? answers.via_aerea || answers.via_aerea_neo : null;
    const emSuporte = Boolean(via) && via !== "ar_ambiente";
    const sedado = subgrupos.has("sedacao");
    if (emSuporte && sedado) return "Em suporte respiratório e sedação,";
    if (emSuporte) return "Em suporte respiratório,";
    if (sedado) return "Em sedação,";
    return null;
  },
...
const SUBGRUPO_POR_ID = {
  ...
  ferida_operatoria_local: "cirurgico", ferida_operatoria_tamanho: "cirurgico",
  ...
  pele: "pele", pele_estadiamento: "pele", pele_neo: "pele", pele_coloracao: "pele",
  hidratacao: "pele",
  pele_lesao_local: "pele", pele_lesao_tamanho: "pele",
  ...
  eliminacoes: "eliminacoes", diurese_volume: "eliminacoes", diurese_aspecto: "eliminacoes",
  ...
  avp_sitio_condicao: "acessos", cvc_sitio_condicao: "acessos", dreno_sitio_condicao: "acessos",
  ...
  tem_resultado: "resultado", resultado_avaliacao: "resultado",
};
...
const CATEGORIA_POR_ID = {
  ...
  ferida_operatoria_local: "geral", ferida_operatoria_tamanho: "geral",
  ...
  pele_coloracao: "exame_fisico", hidratacao: "exame_fisico",
  pele_lesao_local: "exame_fisico",
  pele_lesao_tamanho: "exame_fisico",

  exame_abdominal: "abdome", eliminacoes: "abdome", diurese_volume: "abdome",
  diurese_aspecto: "abdome",
  ...
  avp_sitio_condicao: "dispositivos", cvc_sitio_condicao: "dispositivos",
  dreno_sitio_condicao: "dispositivos",
  ...
  // Vai para bloco próprio do documento, não para a prosa corrida.
  tem_resultado: "outros", resultado_avaliacao: "outros",
};
...
function gerarTexto(context, answers, sequence) {
  ...
  function montarParagrafo(itens, aberturaBruta) {
    ...
    const abertura =
      typeof aberturaBruta === "function" ? aberturaBruta(subgrupos, answers) : aberturaBruta;
