 ...
 *
 *   1. DIAGNÓSTICO NUNCA É INFERIDO. O bloco só é preenchido com o texto que o
 *      enfermeiro escreveu no campo livre. Não há lista de rótulos para
 *      escolher e nenhum valor numérico alimenta esse bloco — não existe
 *      caminho no código para um achado virar diagnóstico.
...
/**
 * Perguntas que alimentam o bloco Diagnóstico. O schema não sugere rótulo:
 * o campo é livre, e só é escrito o que o enfermeiro nomear em palavras.
 */
export const DIAGNOSTICO_IDS = ['tem_diagnostico', 'diagnostico_enfermagem'];
...
export function gerarEvolucaoPE(context, answers, sequence) {
  ...
  const seqDiagnostico = de(DIAGNOSTICO_IDS);
  const seqResultado = de(RESULTADO_IDS);
  const fora = [...INTERVENCAO_IDS, ...DIAGNOSTICO_IDS, ...RESULTADO_IDS];
  ...
  const intervencao = tirarAberturaPaciente(prosaDe(seqIntervencao));
  const diagnostico = prosaDe(seqDiagnostico);
  ...
  const partes = [
    bloco('Dados', dados),
    // Só o que o enfermeiro escreveu. Responder "Não" ao Sim/Não deixa o bloco
    // como "Sem registro" sem virar pendência; não responder nada vira
    // pendência, como qualquer outra pergunta.
    bloco('Diagnóstico de Enfermagem', diagnostico),
