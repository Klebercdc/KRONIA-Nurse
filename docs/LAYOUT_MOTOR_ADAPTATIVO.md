# KRONIA Nurse — Layout do app do motor adaptativo

Especificação de layout da **reestruturação** do app, extraída das telas
oficiais (splash, login, home) e implementada em
`mockups/kronia-nurse-motor-adaptativo.jsx`.
...
Para o layout do app Next.js atual (tema claro, `pages/`, `Layout.tsx`), ver
`docs/LAYOUT.md` e `LAYOUT_ATUAL.md`.
...
`BG` sobrando dos lados em tela larga. Igual ao shell do app atual — nenhuma
tela tem coluna à direita.
...
`store` (topo do arquivo) usa `window.storage` quando existe (ambiente de
